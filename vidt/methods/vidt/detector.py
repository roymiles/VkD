    ...
    def forward(self, samples: NestedTensor):
        ...
        if self.distil:
            ...
            if self.has_projections:
                patch_projectors = [
                    self.patch_projector_1, 
                    self.patch_projector_2, 
                    self.patch_projector_3, 
                    self.patch_projector_4
                ]
                for i in range(len(srcs)):
                    b, c, w, h = srcs[i].shape
                    srcs[i] = srcs[i].permute(0, 2, 3, 1).contiguous().view(b*w*h, c)
                    srcs[i] = patch_projectors[i](srcs[i])

                b, d, c = det_tgt.shape
                det_tgt = det_tgt.contiguous().view(b*d, c)
                det_tgt = self.body_det_projector(det_tgt)

                l, b, d, c = hs.shape
                hs = hs.contiguous().view(l*b*d, c)
                hs = self.neck_det_projector(hs)
