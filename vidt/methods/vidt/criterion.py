    ...
    def forward(self, outputs, targets, distil_tokens=None):
        ...
        if distil_tokens is not None:
            ...
            for patch, teacher_patch in zip(patches, teacher_patches):
                ...
                patch = (patch - patch.mean(0)) / patch.std(0)
                teacher_patch = (teacher_patch - teacher_patch.mean(0)) / teacher_patch.std(0) 
                distil_loss += F.smooth_l1_loss(patch, teacher_patch) 

                # original
                # distil_loss += torch.mean(torch.sqrt(torch.sum(torch.pow(patch - teacher_patch, 2), dim=-1)))

            b, d, c = teacher_body_det.shape
            # body_det = body_det.contiguous().view(b*d, c)
            teacher_body_det = teacher_body_det.contiguous().view(b*d, c).detach()

            # orthogonal modified
            # body_det = projector_body_det(body_det)
            body_det = (body_det - body_det.mean(0)) / body_det.std(0)
            teacher_body_det = (teacher_body_det - teacher_body_det.mean(0)) / teacher_body_det.std(0) 
            distil_loss += F.smooth_l1_loss(body_det, teacher_body_det) 

            # original
            # distil_loss += torch.mean(torch.sqrt(torch.sum(torch.pow(body_det - teacher_body_det, 2), dim=-1)))

            l, b, d, c = teacher_neck_det.shape
            # neck_det = neck_det.contiguous().view(l*b*d, c)
            teacher_neck_det = teacher_neck_det.contiguous().view(l*b*d, c).detach()

            # orthogonal modified
            # neck_det = projector_neck_det(neck_det)
            neck_det = (neck_det - neck_det.mean(0)) / neck_det.std(0)
            teacher_neck_det = (teacher_neck_det - teacher_neck_det.mean(0)) / teacher_neck_det.std(0) 
            distil_loss += F.smooth_l1_loss(neck_det, teacher_neck_det) 
            ...
            l_dict = {'loss_distil': distil_loss}
            losses.update(l_dict)
