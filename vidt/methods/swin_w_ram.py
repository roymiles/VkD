...
def swin_nano(pretrained=None, **kwargs):
    ...
    if pretrained is not None:
        if pretrained == 'imagenet':
            if not os.path.isfile("ckpts/backbones/swin_nano_patch4_window7_224.pth"):
                torch.hub.download_url_to_file(
                        url="https://github.com/naver-ai/vidt/releases/download/v0.1-swin/swin_nano_patch4_window7_224.pth",
                    dst="ckpts/backbones/swin_nano_patch4_window7_224.pth"
                )
            checkpoint = torch.load("ckpts/backbones/swin_nano_patch4_window7_224.pth", map_location="cpu")
...
def swin_tiny(pretrained=None, **kwargs):
    ...
    if pretrained is not None:
        if pretrained == 'imagenet':
            if not os.path.isfile("ckpts/backbones/swin_tiny_patch4_window7_224.pth"):
                torch.hub._download_url_to_file(
                    url="https://github.com/SwinTransformer/storage/releases/download/v1.0.0/swin_tiny_patch4_window7_224.pth",
                    dst="checkpoint.pth"
                )
            checkpoint = torch.load("ckpts/backbones/swin_tiny_patch4_window7_224.pth", map_location="cpu")
            model.load_state_dict(checkpoint["model"], strict=False)
            print('Load the backbone pretrained on ImageNet 1K')
        else:
            checkpoint = torch.load(pretrained, map_location="cpu")
...
def swin_small(pretrained=None, **kwargs):
    ...
    if pretrained is not None:
        if pretrained == 'imagenet':
            if not os.path.isfile("ckpts/backbones/swin_small_patch4_window7_224.pth"):
                torch.hub._download_url_to_file(
                    url="https://github.com/SwinTransformer/storage/releases/download/v1.0.0/swin_small_patch4_window7_224.pth",
                    dst="checkpoint.pth"
                )
            checkpoint = torch.load("ckpts/backbones/swin_small_patch4_window7_224.pth", map_location="cpu")
            model.load_state_dict(checkpoint["model"], strict=False)
            print('Load the backbone pretrained on ImageNet 1K')
        else:
            checkpoint = torch.load(pretrained, map_location="cpu")
